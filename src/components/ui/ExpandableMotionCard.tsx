...
function MotionGridCard({
  ...
  const layoutId = `motion-${artifact.id}`;
    ...
    >
      <div className="relative border border-white/10 bg-black/30 aspect-[4/3] overflow-hidden transition-all duration-500 hover:border-white/20 hover:bg-black/40">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10">
          <div className="h-12 w-12 rounded-full border border-white/20 bg-black/40 backdrop-blur-sm flex items-center justify-center text-white">
        ...
        <div className="absolute top-3 right-3 z-10">
          ...
          >
            {artifact.type === "reel" ? "VERTICAL / 9:16" : "LANDSCAPE / 16:9"}
          </span>
        </div>
...
function MotionModal({
  ...
  const isReel = artifact.type === "reel";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <motion.div
        layoutId={layoutId}
        className={`relative w-full h-auto bg-[#0c0c0f]/95 overflow-hidden border border-white/10 z-10 flex ${
          isReel ? "flex-col md:flex-row max-w-[900px] max-h-[85vh]" : "flex-col max-w-[1000px] max-h-[90vh]"
        }`}
        style={{
          boxShadow: `
      ...
      >
        ...
        <div 
          className={`relative shrink-0 overflow-hidden ${
            isReel ? "w-full md:w-[380px] h-[50vh] md:h-full" : "w-full flex-none"
          }`}
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="h-16 w-16 rounded-full border border-white/20 bg-black/40 backdrop-blur-sm flex items-center justify-center text-white/80">
              <Play size={24} fill="currentColor" className="ml-1" />
            </div>
          </div>

          {/* Image overlays */}
          <div className={`absolute inset-0 ${isReel ? "bg-gradient-to-t from-[#0c0c0f] via-transparent to-transparent md:bg-gradient-to-r md:from-transparent md:to-[#0c0c0f]/30" : "bg-gradient-to-t from-[#0c0c0f] via-transparent to-transparent"}`} />
        ...
        {/* ─── Dossier content ─── */}
        <div className={`flex-1 flex flex-col min-w-0 overflow-y-auto expandable-card-scrollbar ${isReel ? "" : "border-t border-white/10"}`}>
          <div className="p-5 sm:p-7 md:p-8 flex flex-col gap-6">
            ...
            <div className={`grid grid-cols-1 ${isReel ? "" : "md:grid-cols-2"} gap-6`}>
              {/* Overview section */}
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div
              ...
              className={`mt-2 flex items-center justify-between border-t border-white/8 pt-5 ${isReel ? "mt-auto" : ""}`}
            >
...
export function ExpandableMotionCards({
  artifacts,
  accent,
  sectionDelay = 0.05,
  prefix = "VD",
